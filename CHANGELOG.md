...
- [nj][data] Added ability to specify canonical urls for specific pages with Matter Data, using following structure:
...
- [nj] Added structured data for `Breadcrumb` component according to [Google guidlines](https://developers.google.com/search/docs/data-types/breadcrumbs).
...
- [sass] Updated Ekzo to version 2.4.3.
- [sass][grunt] Updated Sass files to use same comments headers as Ekzo 2.4.3 — 80 chars long.
- [sass] Update `stylelint-disable` to use changed Stylelint 7.12.0 rules.
...
- [modules][nj] Removed `isCaller` from `nunjuck-render` method, since is is no longer needed to make adjustments to input based on whether it is macro's caller or no. This also means that Nunjucks `render()` filter no longer accepts this parameter too.
...
- [nj] Fixed `rootTitle` in `Breadcrumb` component formatted with `sprintf` twice.
